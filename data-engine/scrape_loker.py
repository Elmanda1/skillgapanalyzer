...
def build_aggregate():
    ...
    if AGGREGATE_PATH.exists():
        try:
            ...
            print(f"[SNAPSHOT] Salinan agregat lama disimpan ke {snapshot_path}", flush=True)
        except Exception as e:
            print(f"[SNAPSHOT] Gagal membuat snapshot: {e}", flush=True)

    # 2. Guard Rail Sanity Check
    n_old = len(old_records)
    n_new = len(records)
    if n_old > 0 and n_new < (n_old * 0.50):
        print(f"[WARNING] Guard Rail Triggered: Jumlah lowongan baru ({n_new}) turun drastis dari sebelumnya ({n_old}). Skipped full overwrite & merged.", flush=True)
