...
"""
    ...
    def _extract_fallback(self, raw_html: str, trimmed_text: str, url: str) -> JobExtractionSchema:
        ...
        Extracts title, company, location, and skills using BeautifulSoup & regex.
        """
        soup = BeautifulSoup(raw_html, "html.parser")
        ...
        salary_match = re.search(r'Rp\s*([\d.]+)\s*[-–]\s*Rp?\s*([\d.]+)', trimmed_text, re.I)
        if salary_match:
            try:
        ...
        # 6. Skill keywords discovery from text
        common_skills = [
